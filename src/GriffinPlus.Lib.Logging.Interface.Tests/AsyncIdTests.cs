...
namespace GriffinPlus.Lib.Logging
{
	...
	public class AsyncIdTests
	{
		...
		private async Task Current_ReuseIdAsync(uint expectedId)
		{
		...
		private void Current_TestWrapAround()
		{
			// set the internal id counter to the greatest possible value
			unchecked
			{
				var field = typeof(AsyncId).GetField("sAsyncIdCounter", BindingFlags.NonPublic | BindingFlags.Static);
