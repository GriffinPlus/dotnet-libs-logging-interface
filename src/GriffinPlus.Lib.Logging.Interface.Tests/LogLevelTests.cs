...
public class LogLevelTests
{
	...
	private readonly struct LogLevelItem(int id, string name)
	{
	...
	public void KnownLevelsByName()
	{
		...
		Dictionary<string, LogLevel> remaining = LogLevel.KnownLevelsByName.ToDictionary(
			x => x.Key,
			x => x.Value);
		...
		for (int i = 0; i < sExpectedKnownLogLevels.Length; i++)
		{
			...
			LogLevel level = LogLevel.KnownLevelsByName[expectedName];
