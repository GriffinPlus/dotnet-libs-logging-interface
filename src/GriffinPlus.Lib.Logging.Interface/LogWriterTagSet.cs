...
namespace GriffinPlus.Lib.Logging
{
	...
	/// <summary>
	/// A sorted collection of log writer tags (immutable, thread-safe).
	/// 
	...
	public sealed class LogWriterTagSet : ITagSet, IReadOnlyList<LogWriterTag>, IEquatable<LogWriterTagSet>
	{
		...
		/// </summary>
		private class TagComparer : IComparer<LogWriterTag>, IEqualityComparer<LogWriterTag>
		{
			...
			/// <param name="x">Tag to compare.</param>
			/// <param name="y">Tag to compare with.</param>
			/// <returns><c>true</c> if both tags are equal; otherwise <c>false</c>.</returns>
		...
		private static readonly TagComparer        sOrdinalComparer           = new TagComparer(StringComparison.Ordinal);
		private static readonly TagComparer        sOrdinalIgnoreCaseComparer = new TagComparer(StringComparison.OrdinalIgnoreCase);
		private static readonly List<LogWriterTag> sEmpty                     = new List<LogWriterTag>();
		...
		public LogWriterTagSet(IEnumerable<LogWriterTag> tags)
		{
			...
			mTags = new List<LogWriterTag>(new HashSet<LogWriterTag>(tags, sOrdinalComparer));
		...
		public bool Equals(LogWriterTagSet other)
		{
			...
			return mTags.SequenceEqual(other.mTags, sOrdinalComparer);
		}
